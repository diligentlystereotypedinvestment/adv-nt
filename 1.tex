...
\begin{exercise}
	\begin{lemma}
		$P = \{b^a, a\in\N}$ forms a group under $\cdot$ where $b\in\Z/m\Z$.
		\begin{proof}
			First we can show closure. Suppose we have two elements of $P$, $b^a$ and $b^c$ for $a,b \in\N$. Then $b^a \cdot b^c = b^{a+c}$, and since $a+c \in\N, \, b^{a+c} \in P$ (note that adding the exponent rule works since when we multiply equivalence classes, we can pick any arbitrary element of the equivalence class. Thus we can simply pick $b^a$ each time, and do multiplication in the integers).

			Then we can show there is an identity, which we claim is 1, through contradiction.
		\end{proof}
	\end{lemma}
	\begin{proof}
